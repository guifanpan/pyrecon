...
from pyrecon.iterative_fft_2lpt import IterativeFFTReconstruction2LPT
...
recon2 = IterativeFFTReconstruction2LPT(
    f=0.8, bias=2.0, nmesh=128,
    boxsize=[1000., 1000., 1000.],
    boxcenter=[500., 500., 500.]
)
# 正确传入位置和权重
recon2.assign_data(data_pos, weights=data_w)     # 或只传 data_pos，默认权重为 1
recon2.assign_randoms(rand_pos, weights=rand_w)

# 计算密度对比（需要先调用）
recon2.set_density_contrast(smoothing_radius=15.0)
...
# 读取位移场
pos = np.column_stack([data_x, data_y, data_z])   # 与 data_pos 相同，也可直接用 data_pos
shifts2 = recon2.read_shifts(pos, field='disp')
print("+2LPT shifts RMS:", np.sqrt(np.mean(shifts2**2)))

# # 初始化（参数与 IterativeFFTReconstruction 相同）
# recon = IterativeFFTReconstruction2LPT(
#     f=0.8, bias=2.0, nmesh=256, boxsize=1000.,
#     D2=-0.35,          # 可选，若不提供则根据 omega_m 自动计算
#     omega_m=0.31
# )

# # 分配数据
# recon.assign_data(data_pos, data_weights)
# recon.assign_randoms(rand_pos, rand_weights)
# recon.set_density_contrast(smoothing_radius=15.)

# # 运行迭代（与原来完全一致）
# recon.run(niterations=3)  # 内部会调用我们重写的 _compute_psi

# # 读取位移（现在包含 2LPT 修正）
# shifts = recon.read_shifts(positions, field='disp')